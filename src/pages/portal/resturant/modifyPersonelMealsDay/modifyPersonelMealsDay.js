...
import {
  EditPersonelMealDay,
  AddPersonelMealDay,
  BulkCurrentMonthAddPersonelMealDays,
  BulkCurrentMonthEditPersonelMealDays,
...
const ModifyPersonelMealsDay = (props) => {
      ...
      <Container>
        ...
        currentMonthDates.filter(currentMonthDate => (Number(getPersianMonthWeekNo(currentMonthDate.date)) === 1))
                                    .map((currentMonthDate, index) => (
                                      ...
                                      />
                                  //   meals={filteredMeals(currentMonthDate.date)} 
                                  //   mealsDays={filteredMealsDay(currentMonthDate.date)} 
                                  //   personelMealDays={filteredPersonelMealsDay(currentMonthDate.date)}                                       
            )) : ""}
          </Card>
          <Card className="card-res-week">
        {currentMonthDates && currentMonthDates.length > 0 ? 
            ...
            <Row>
              <Col style={{textAlign:'center', width:'150px'}}>
                ...
                {/* {!state.isSaved ?
                  <Button color="primary" style={{margin:"1em auto 1em auto", width:'100px'}} onClick={() => save()} >ذخیره</Button>
                  :
                  <Button color="primary" style={{margin:"1em auto 1em auto", width:'100px'}} disabled >ذخیره</Button>
                } */}
              </Col>
            </Row>
        </Card>
...
const mapDispatchToProps = dispatch => {
  return {
    bulkEditPersonelMealDays: (personelMealDays) => {
        dispatch(BulkCurrentMonthEditPersonelMealDays(personelMealDays))
    },
    editPersonelMealDay: (model) => {
      dispatch(EditPersonelMealDay(model))
    }, 
    bulkAddPersonelMealDays: (personelMealDays) => {
        dispatch(BulkCurrentMonthAddPersonelMealDays(personelMealDays))
    },
...
export default connect(mapStateToProps, mapDispatchToProps)(ModifyPersonelMealsDay);
