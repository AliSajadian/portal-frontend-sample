...
import { GetAsftDayMealsStatistics } from "../../../../redux/actions/personelMealDayActions";
import './index.css';
...
const AsftDayMealsStatistics = (props) => {
    ...
    const showAsftDayMealsStatistics = (year, month, day) => {
        let date = getGregorianDateEx(year, month, day)
        props.getAsftDayMealsStatistics(date)
        setDate(date)
    }

    return (
        <Card className="card3D" style={{direction:'rtl'}}>{props.getPersianMonth(new Date())}
        <CardHeader>
            <CardTitle tag="h2">
        ...
        <CardBody >
            <Container>
                <Container>
                    ...
                    <Row>
                        <Col xl="12" style={{testAlign:'center'}}>
                            <Button color="primary" style={{margin:"1em auto 1em auto", width:'100px'}} onClick={() => showAsftDayMealsStatistics(year, month, day)} >جستجو</Button>
                        </Col>
                    </Row> 
                </Container>    
                    ...
                    <CardBody >
                        ...
                        {props.departments.filter(department => props.companies.filter(
                            ...
                            <Card key={index} className="card_item">
                                ...
                                <CardBody >
                                {props.asftDayMealsStatistics.filter(asftDayMealsStatistic => asftDayMealsStatistic.section_type === 1 &&
                                    asftDayMealsStatistic.section === department.name).map((asftDayMealsStatistic, index) => (
                                    <Container key={index}>
                                        <label className='meal_no'>{asftDayMealsStatistic.meal_no}</label>
                                        <label>{' :    '}</label>
                                        <label className='meal_name'>{asftDayMealsStatistic.meal_name}</label>
                                        <br/>
                                    </Container>
                                    ))}
                                </CardBody>
                ...
                : ''}
                {date !== '' && props.projects && props.projects.length > 0 ?
                <Card className='container_item'>
                    <CardHeader>
                        <CardTitle tag="h3">
                            آمار غذاهای روزانه پروژه ها 
                        </CardTitle>
                    </CardHeader>
                    <CardBody >
                    ...
                    <Container >
                        {props.projects.map((project, index) => (
                            <Card key={index} className="card_item">
                                ...
                                <CardBody >
                                {props.asftDayMealsStatistics.filter(asftDayMealsStatistic => asftDayMealsStatistic.section_type === 2 &&
                                    asftDayMealsStatistic.section === project.name).map((asftDayMealsStatistic, index) => (
                                    <Container key={index}>
                                        <label className='meal_no'>{asftDayMealsStatistic.meal_no}</label>
                                        <label>{' :    '}</label>
                                        <label className='meal_name'>{asftDayMealsStatistic.meal_name}</label>
...
const mapStateToProps = store => {
  return {
    ...
    asftDayMealsStatistics: store.personelMealDays.asftDayMealsStatistics,
...
const mapDispatchToProps = dispatch => {
  return {
    getPersianMonth: date => {
        ...
        getAsftDayMealsStatistics: (date) => dispatch(GetAsftDayMealsStatistics(date)),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(AsftDayMealsStatistics);
