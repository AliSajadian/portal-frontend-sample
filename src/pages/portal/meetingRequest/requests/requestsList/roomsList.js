...
class RequestList extends Component {
  ...
  showMeetingRequestofThisDate = (day, month, year) => {
    let date = this.getGregorianDateEx(year, month, day)
    this.props.getRequestofDateList(date, department)

    this.setState({ 
      mealsDay: this.props.requests,
    })
    // console.log('**************************Button***************************')
    ...
    return (
      <Card style={{direction:'rtl'}}>
        <Container>
          ...
          // <Row></Row>
          this.state.requests.map((index, request) => (
            <Card>
              <Row key={index}>
                <Col xl="8">
                ...
                <Col xl="8">
                  {/* {console.log('*** mealDay.resturant_meal__name: ', mealDay.resturant_meal__name)} */}
                  <Label>{request.startHour}</Label>
                </Col>
                <Col xl="8">
                  {/* {console.log('*** mealDay.resturant_meal__name: ', mealDay.resturant_meal__name)} */}
                  <Label>{request.endHour}</Label>
                </Col>
              </Row>)
            </Card>
          )) : ''}
...
const mapDispatchToProps = dispatch => {
  return {
    getRequestofDateList: date => {
      dispatch(GetRequestofDateList(date))}, 
  };
};
