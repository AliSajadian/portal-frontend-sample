...
export const GetProjectsList = () => {
    return dispatch => {
        axios.get('http://172.20.20.45:90/api/projects/')
        .then((response) => {
            dispatch({
...
export const LoadRelatedProjectInfoCard = (projectId) => {        
    return (dispatch) =>  {
        axios.get(`http://172.20.20.45:90/api/projects/${projectId}`)
        .then((response) => {
            dispatch({
...
export const RemoveProject = (id) => {
    return dispatch => {
        axios.delete(`http://172.20.20.45:90/api/projects/${id}`)
            .then(() => {
                dispatch({
...
export const AddProject = project => {
    return dispatch => {
        axios.post("http://172.20.20.45:90/api/projects/", project)
            .then(res => {
                dispatch({
...
// GET Project MODAL
 export const GetProjectsModal = (id) => {
    return dispatch => {
        axios.get(`http://172.20.20.45:90/api/projects/${id}`)
        .then((response) => {
            dispatch({
...
export const EditProject = project => {
    return dispatch => {
        axios.put(`http://172.20.20.45:90/api/projects/${project.id}/`, project)
            .then(resonse => {
                dispatch({
