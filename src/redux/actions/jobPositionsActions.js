...
export const GetJobPositionsList = () => {
    return dispatch => {
        axios.get('http://172.20.20.45:90/api/jobPositions/')
        .then((response) => {
            dispatch({
...
export const LoadRelatedJobPositionInfoCard = (jobPositionId) => {        
    return (dispatch) =>  {
        axios.get(`http://172.20.20.45:90/api/jobPositions/${jobPositionId}`)
        .then((response) => {
            dispatch({
...
export const RemoveJobPosition = (id) => {
    return dispatch => {
        axios.delete(`http://172.20.20.45:90/api/jobPositions/${id}`)
            .then(() => {
                dispatch({
...
export const AddJobPosition = jobPosition => {
    return dispatch => {
        axios.post("http://172.20.20.45:90/api/jobPositions/", jobPosition)
            .then(res => {
                dispatch({
...
// GET JobPosition MODAL
 export const GetJobPositionsModal = (id) => {
    return dispatch => {
        axios.get(`http://172.20.20.45:90/api/jobPositions/${id}`)
        .then((response) => {
            dispatch({
...
export const EditJobPosition = jobPosition => {
    return dispatch => {
        axios.put(`http://172.20.20.45:90/api/jobPositions/${jobPosition.id}/`, jobPosition)
            .then(resonse => {
                dispatch({
