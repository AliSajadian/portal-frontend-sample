...
export const GetDoctorTypesList = () => {
    return dispatch => {
        axios.get('http://172.20.20.45:90/api/doctorTypes/')
        .then((response) => {
            dispatch({
...
export const LoadRelatedDoctorTypeInfoCard = (doctorTypeId) => {        
    return (dispatch) =>  {
        axios.get(`http://172.20.20.45:90/api/doctorTypes/${doctorTypeId}`)
        .then((response) => {
            dispatch({
...
export const RemoveDoctorType = (id) => {
    return dispatch => {
        axios.delete(`http://172.20.20.45:90/api/doctorTypes/${id}`)
            .then(() => {
                dispatch({
...
export const AddDoctorType = doctorType => {
    return dispatch => {
        axios.post("http://172.20.20.45:90/api/doctorTypes/", doctorType)
            .then(res => {
                dispatch({
...
// GET DOCTORTYPE MODAL
 export const GetDoctorTypesModal = (id) => {
    return dispatch => {
        axios.get(`http://172.20.20.45:90/api/doctorTypes/${id}`)
        .then((response) => {
            dispatch({
...
export const EditDoctorType = doctorType => {
    return dispatch => {
        axios.put(`http://172.20.20.45:90/api/doctorTypes/${doctorType.id}/`, doctorType)
            .then(resonse => {
                dispatch({
