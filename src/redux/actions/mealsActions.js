...
export const GetMealsList = () => {
    return dispatch => {
        axios.get('http://172.20.20.45:90/api/meals/')
        .then((response) => {
            dispatch({
...
export const LoadRelatedMealInfoCard = (mealId) => {        
    return (dispatch) =>  {
        axios.get(`http://172.20.20.45:90/api/meals/${mealId}`)
        .then((response) => {
            dispatch({
...
export const RemoveMeal = (id) => {
    return dispatch => {
        axios.delete(`http://172.20.20.45:90/api/meals/${id}`)
            .then(() => {
                dispatch({
...
export const AddMeal = Meal => {
    return dispatch => {
        axios.post("http://172.20.20.45:90/api/meals/", Meal)
            .then(res => {
                dispatch({
                type: types.ADD_MEAL,
                payload: res.data
...
// GET MEAL MODAL
 export const GetMealsModal = (id) => {
    return dispatch => {
        axios.get(`http://172.20.20.45:90/api/meals/${id}`)
        .then((response) => {
            dispatch({
...
export const EditMeal = Meal => {
    return dispatch => {
        axios.put(`http://172.20.20.45:90/api/meals/${Meal.id}/`, Meal)
            .then(resonse => {
                dispatch({
